...
class PiMarkdownProvider {
  ...
  interrupt(value) { const ticket = this.resolveTicket(value); const child = this.children.get(ticket.id); if (!child) return { interrupted: false, id: ticket.id, filePath: ticket.filePath }; child.kill('SIGTERM'); this.children.delete(ticket.id); this.runtimeStatuses.set(ticket.id, 'waiting'); this.appendRun({ event: 'interrupted', ticketId: ticket.id }, ticket.cwd); return { interrupted: true, id: ticket.id, filePath: ticket.filePath }; }
