...
export function AgentDialogs({ mode, open = Boolean(mode), onClose, folderId, threadId, onSaved }: AgentDialogsProps) {
  ...
  const close = () => { setError(""); setConfirmation(""); onClose?.(); };
  ...
  const warning = provider === "pi"
    ...
    : provider === "claude"
    ? mode === "delete" ? `Permanently delete this Claude Code context and ${descendantIds.length} mapped child context${descendantIds.length === 1 ? "" : "s"}? Because Claude Code has no provider delete API, Constellation will delete only the canonical discovered local transcript JSONL file${descendantIds.length === 0 ? "" : "s"} for this context and its mapped descendants. It will not touch project files, memories, settings, credentials, or unrelated caches. Stop active agents first. This cannot be undone.` : "Archive this Claude Code session in Constellation? Its original local Claude history remains untouched."
