...
export function ListView({ onNewAgent }: ListViewProps) {
  ...
  return <section className={styles.view} aria-labelledby="list-heading">
    ...
    <div className={styles.tableWrap}><table><caption className={styles.srOnly}>Threads grouped by folder</caption><thead><tr><th scope="col">Task</th><th scope="col">Provider</th><th scope="col">Status</th><th scope="col">Parent</th><th scope="col">Model</th><th scope="col">Runtime</th><th scope="col">Attention</th></tr></thead><tbody>{groups.map(({ folder, threads: group }) => <>{<tr key={`${folder.id}-group`} className={styles.group}><th colSpan={7} scope="colgroup"><button onClick={() => selectFolder(folder.id)}><span className={styles.dot} style={{ background: folder.accent }} />{folder.name}<span className={styles.path}>{folder.path}</span></button></th></tr>}{group.map((thread) => { const parent = thread.parentId ? threads[thread.parentId] : undefined; const provider = getProvider(thread); return <tr key={thread.id} className={selectedThreadId === thread.id ? styles.selected : ""}><td><button className={styles.task} onClick={() => selectThread(thread.id)}><span className={`${styles.statusDot} ${styles[thread.status]}`}><StatusIcon status={thread.status} /></span><span><strong>{thread.title}</strong><small>{thread.key} · {thread.summary}</small></span></button></td><td><span className={styles.provider} style={{ color: provider.color, borderColor: provider.color }}><i>{provider.short}</i>{provider.label}</span></td><td><span className={`${styles.statusText} ${styles[thread.status]}`}><StatusIcon status={thread.status} />{statusLabels[thread.status]}</span></td><td>{parent ? <span className={styles.parent}><UserRound />{parent.title}</span> : <span className={styles.root}>Root task</span>}</td><td>{thread.model}<small className={styles.meta}>{thread.reasoningEffort} effort</small></td><td>{duration(thread)}</td><td>{thread.attention ? <span className={styles.attention}>{thread.attention.kind}</span> : <span className={styles.muted}>—</span>}</td></tr>; })}</>)}</tbody></table></div>
  </section>;
}
