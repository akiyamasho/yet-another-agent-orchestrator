...
- `Delete permanently` is a separate, explicit action with typed `DELETE` confirmation in every mode, and a hard precondition that all sessions Constellation can identify as running among the selected agent and its descendants have stopped. Other clients’ in-memory runtime state is not observable. For Codex it calls the official App Server `thread/delete` mutation and recursively deletes the selected thread’s mapped descendants, with no Undo. For Claude Code, which has no supported delete API, the dialog adds a “Delete permanently instead of moving to Trash” checkbox, unticked by default: by default `deleteSession` moves only canonical provider-discovered `.jsonl` transcript files under `~/.claude/projects` for the selected session and recursively mapped descendants to the macOS Trash via `shell.trashItem`, recoverable until the Trash is emptied and reclaiming disk space only then; ticking the checkbox passes `{ permanent: true }`, which unlinks those files immediately instead, with no Undo. Either mode deletes descendants before parents and reports the summed `bytesFreed` plus whether the transcripts were moved to the Trash or deleted permanently. It must never touch project files, `CLAUDE.md`/memories, credentials, settings, or unrelated caches. Deleting a main agent always cascades to its subagents.
- Claude Code uses Archive and title changes as app-local overlay state; those actions do not modify provider history. The guarded `Delete permanently` flow is the only exception to normal read-only Claude history discovery.
