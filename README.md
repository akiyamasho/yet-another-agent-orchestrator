...
- In worktree mode, completed worker changes can be automatically integrated only after review: Constellation verifies the exact reviewed source commit, serializes integrations with a tokenized lock, preserves project and conflict data, and records the merge and any parent-completion commit. Source branches are retained. Root mode is unchanged and does not perform worktree integration.
- Review, integration, commit, conflict, and cleanup outcomes are visible in the Inspector. After a restart, reconciliation is conservative; the Inspector provides explicit retry-review, retry-integration, and cleanup controls. Automatic cleanup removes only verified-clean, deterministic worktrees owned by Constellation, and never uses force cleanup.
...
- Pi recovery IPC is deliberately narrow: it accepts only a validated run ID, resolves the registered project root in the main process, and exposes review, integration, and cleanup actions without filesystem paths from the renderer. Pi integration and recovery do not use Git reset, clean, stash, abort, or force cleanup; conflicts and data remain available for manual attention.
